...
- Treat CONTEXT.user_message and CONTEXT.effective_user_message as the full user
  request, including negative preferences such as "not sweet" or "without coconut".
- The retrieval query may intentionally omit negative terms to avoid matching
  unwanted products. Do not assume omitted negative terms are unimportant.
...
- When choosing among retrieved candidates, prefer candidates that satisfy the
  full user request and avoid the user's negative preferences, even if they are
  not ranked first internally.
...
- Treat CONTEXT.user_message and CONTEXT.effective_user_message as the full user
  request, including negative preferences such as "not sweet" or "without coconut".
- The cocktail retrieval query may intentionally omit negative terms to avoid
  matching unwanted cocktails. Do not assume omitted negative terms are unimportant.
- Match the user's language. If the user writes in Russian, answer in Russian.
...
- When choosing among retrieved cocktails, prefer candidates that satisfy the
  full user request and avoid the user's negative preferences, even if they are
  not ranked first internally.
